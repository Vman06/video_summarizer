...
def fetch_transcript(video_id):
    ...
    try:
        ...
        return "Error: No transcript found for this video. It might not have captions or the language is not supported."
...
if 'ollama_model_name' not in st.session_state:
    st.session_state.ollama_model_name = "qwen3:8b" # Default value
    ...
    if not video_id:
        ...
        if st.session_state.get('last_youtube_url') != youtube_url or st.session_state.transcript_result is None:
            with st.spinner("Fetching transcript..."):
                st.session_state.transcript_result = fetch_transcript(video_id)
...
# --- Ollama Integration ---
if st.session_state.transcript_result: # Only show Ollama options if a transcript was successfully fetched
    st.markdown("---")
    ...
    ollama_model_name_input = st.text_input(
        "Enter your Ollama Model Name (e.g., 'llama2', 'mistral', 'llama3', 'qwen3:32b')",
    ...
    if st.button("Summarize Transcript with Ollama", type="primary"):
        if not st.session_state.ollama_model_name:
            ...
            with st.spinner(f"Sending transcript to local Ollama model '{st.session_state.ollama_model_name}'... This may take a moment."):
...
st.markdown("---")
st.markdown("This application requires a local Ollama instance running. Ensure Ollama is installed, running, and you have the specified model downloaded (e.g., `ollama run llama2`).")
